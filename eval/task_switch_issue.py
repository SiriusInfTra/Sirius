...
def run(system: System, hyper_workload:HyperWorkload, num_model:int, req_per_sec:int, tag:str):
    ...
    system.launch("taskswitch-issue", f"{tag}-{req_per_sec_str}", time_stamp=True, infer_model_config=infer_model_config)
    hyper_workload.set_infer_workloads(PoissonInferWorkload(
...
def main():
    ...
    for num_model, req_per_sec, train_thread_percent in parmas:
        ...
        run(system, workload, num_model, req_per_sec, "ideal")
    ...
    run(system, workload, 1, 0.1, "taskswitch")
    run(system, workload, 1, 0.2, "taskswitch")
    run(system, workload, 1, 0.5, "taskswitch")

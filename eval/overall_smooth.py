...
if run_strawman:
    ...
    with mps_thread_percent(eval_config.heavy_mps_infer):
        ...
        system = System(mode=System.ServerMode.ColocateL2, use_sta=False, mps=True, use_xsched=False, port=eval_config.server_port,
                        ...
                        has_warmup=True, infer_model_max_idle_ms=5000, memory_pool_policy=System.MemoryPoolPolicy.BestFit,
...
# [500, 1000, 2000, 4000, 5000, 8000, 1000*1000]
if run_dynamic_overhead:
    for max_idle_ms in [2000, 4000, 5000, 8000, (60 + eval_config.increase_time + 30)]:
        num_worker = 0
        with mps_thread_percent(eval_config.heavy_mps_infer):
            client_model_list, server_model_config = InferModel.get_multi_model(eval_config.model_list, eval_config.heavy_num_model, num_worker)
            hyper_workload = smooth(rps=eval_config.heavy_rps, client_model_list=client_model_list, infer_only=False)
            system = System(mode=System.ServerMode.ColocateL1, use_sta=True, mps=True, use_xsched=True, 
                    cuda_memory_pool_gb="13.5", ondemand_adjust=True, train_memory_over_predict_mb=1500,
                    train_mps_thread_percent=eval_config.heavy_mps_train,
                    infer_model_max_idle_ms=max_idle_ms, has_warmup=True, port=eval_config.server_port)
            tag = f"{max_idle_ms}" if max_idle_ms < (60 + eval_config.increase_time + 30)*1000 else "inf"

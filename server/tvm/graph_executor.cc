...
void GraphExecutor::PipelineRun() {
  ...
  for (size_t i = 0; i < op_execs_.size(); i++) {
    if (op_execs_[i]) {
      ...
      op_execs_[i]();
      DeviceAPI::Get(factory_.devices_[0], run_stream_);
    }
  }
}
...
void GraphExecutor::SetupStorage() {
  ...
  // for (const auto &pit : factory_.pool_entry_) {
  for (size_t sid = 0; sid < factory_.pool_entry_.size(); sid++) {
    ...
    // op_node_storage_id_[pit.param_data_entry] = storage_pool_.size() - 1;
  ...
  for (size_t i = 0; i < data_entry_.size(); i++) {
    ...
    auto a = storage->CreateView(factory_.attrs_.shape[i], vtype[i]);
    data_entry_[i] = storage->CreateView(factory_.attrs_.shape[i], vtype[i]);

...
torch.onnx.export(inception_v3, torch.rand(batch_size, 3, 299, 299), 
                  f"{model_store}/inception_v3.onnx", verbose=True,
...
tvmc_model = tvmc.load(f"{model_store}/inception_v3.onnx", 
                       shape_dict={'input':[batch_size, 3, 299, 299]})
tvmc.compile(tvmc_model=tvmc_model, target='cuda', package_path=f"{model_store}/inception_v3-tvm.tar")


model_store_path = f'models/inception_v3-b{batch_size}' 
pathlib.Path(model_store_path).mkdir(parents=True, exist_ok=True)
tarfile.open(f"{model_store}/inception_v3-tvm.tar").extractall(model_store_path)

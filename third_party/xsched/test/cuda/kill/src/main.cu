...
#define SLEEP_US            800
#define BLOCK_TIME_MAX_US   1000

void TestBlockPreempt(uint64_t us, cudaStream_t stream)
{
    LoopRunner runner;
...
void TestDeadLoopPreempt(cudaStream_t stream)
{
    LoopRunner runner;
    DataProcessor<int64_t> preempt_latency;
    DataProcessor<int64_t> restore_latency;
...
int main()
{
    ...
    CudaXQueueCreate(stream, PREEMPT_MODE_INTERRUPT, 16, 8);

    for (size_t i = 0; i <= BLOCK_TIME_MAX_US; i += 10) {
        TestBlockPreempt(i, stream);
    }

    TestDeadLoopPreempt(stream);
}
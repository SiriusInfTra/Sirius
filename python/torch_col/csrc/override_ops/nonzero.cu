#ifndef COLSERVE_NONZERO_H
#define COLSERVE_NONZERO_H
...
void nonzero_cuda_out_impl(const at::Tensor& self, at::Tensor& out){
...
// compute number of nonzero elements
  ...
  auto num_nonzeros_ts = MakeColTensorEmpty({sizeof(int)}, at::kByte);
  void* num_nonzeros = num_nonzeros_ts.data_ptr();
  ::cub::TransformInputIterator<bool, NonZeroOp<scalar_t>, scalar_t*> itr(self_.data_ptr<scalar_t>(), NonZeroOp<scalar_t>());
  ::cub::DeviceReduce::Sum(nullptr, temp_storage_bytes, itr, (int*)num_nonzeros, N, stream);
  auto temp_storage_ts = MakeColTensorEmpty({(int64_t)temp_storage_bytes}, at::kByte);
  void* temp_storage = temp_storage_ts.data_ptr();
  ::cub::DeviceReduce::Sum(temp_storage, temp_storage_bytes, itr, (int*)num_nonzeros, N, stream);
  int num_nonzeros_h;
  at::cuda::memcpy_and_sync(&num_nonzeros_h, num_nonzeros, sizeof(int), cudaMemcpyDeviceToHost, stream);
  ...
  at::Tensor out_temp = need_to_copy ?
      MakeColTensorEmpty({self.dim(), num_nonzeros_h}, out.options()) :
      out.resize_({self.dim(), num_nonzeros_h});
  ...
  if (self.dim() > 0) {
    ...
    ::cub::DeviceSelect::Flagged(nullptr, temp_storage_bytes, counting_itr, itr,
      out_temp.data_ptr<int64_t>(), (int*)num_nonzeros, N, stream);
    // temp_storage = allocator.allocate(temp_storage_bytes);
    temp_storage_ts = MakeColTensorEmpty({(int64_t)temp_storage_bytes}, at::kByte);
    temp_storage = temp_storage_ts.data_ptr();
    ::cub::DeviceSelect::Flagged(temp_storage, temp_storage_bytes, counting_itr, itr,
      out_temp.data_ptr<int64_t>(), (int*)num_nonzeros, N, stream);
